...
- **Gate is client-side only** (index): `#gate` overlays the page on load, `body` overflow is locked, the signup form validates "all fields filled" and `age >= 16` in JS, then `openGate()` plays the ACCESS GRANTED stamp. Login link is a toast stub. Nothing persists, no storage keys. `landing()` runs when the gate lifts: it strips any `#hash` left by EXPLORE or nav links, scrolls to the very top, and `history.scrollRestoration` is `manual` so a reload never lands mid-page. Client asked for this explicitly: entering through the gate always starts at the hero, even from a sub-page link like `index.html#about`. Only `?preview&at=` targets a section.
